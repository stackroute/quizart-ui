...
handleOption=()=>{
  ...
  for(var variable in option)
    {
      var index=optionsTemp.indexOf(option[variable]);
...
handleVariableMeaning=()=>{
  Request.post('http://localhost:8081/getVariableMeaning')
    ...
    .end((err, res) => {
      if (res.status===200) {
        variableMeaningArray.push(<Card style={{height:70,width:"90%",margin:"auto"}}> <h4 style={{textAlign:"center",paddingTop:20}}>Hey! Lemme Know What Did You Mean By "  {this.state.selectedVariable} "</h4>
              </Card>);
        if(res.body===null){
          res.body = JSON.parse(res.text);
          res.body.search.map(function(data){
            var text=data.label+" - "+data.description;
            var key=0;
            var variableMeaningTemp=[];
            variableMeaningTemp.push(<List style={{margin:"0% 10% 0% 10%"}}>
            <ListItem key={data.description} primaryText={text}
                    onClick={() => { this.handleSelectedVariableMeaning(text) }} style={{backgroundColor:'#B3E5FC',margin:'5px',textAlign:'center',color:'#3F51B5'}}/></List>);
...
handleOptionMeaning=()=>{
  Request.post('http://localhost:8081/getOptionMeaning')
    ...
    .end((err, res) => {
      if (res.status===200) {
        optionMeaningArray.push(<Card style={{height:80,width:"90%",margin:"auto"}}> <h4 style={{textAlign:"center",paddingTop:20}}>Ok! I got that.... Now Which of following best match " {this.state.selectedOption} " in Context of " {this.state.selectedVariable} "</h4>
                </Card>);
        if(res.body===null){
    res.body = JSON.parse(res.text);
    res.body.search.map(function(data){
      ...
      var key=0;
        ...
        optionMeaningTemp.push(<List style={{margin:"0% 10% 0% 10%"}}>
             <ListItem key={data.description} primaryText={text}  value={text} onClick={() => { this.handleSelectedOptionMeaning(text) }}  style={{backgroundColor:'#B3E5FC',margin:'5px',textAlign:'center',color:'#3F51B5'}}/></List>);
...
handleFinalMeaning=()=>{
  var key=0;
      finalMeaningArray.push(<Card style={{height:100,width:"90%",margin:"auto"}}> <h4 style={{textAlign:"center",paddingTop:20}}>Trying To Figure Out What Kind Of Entity " {this.state.selectedVariable} " is... Select The Description Which Matches Best</h4>
            </Card>);
  ...
  var key=0;
    ...
    var optionsTemp=[];
     ...
     {arr}
      ...
      noofoptionsArray.push(<RaisedButton label="Proceed" primary={true} disabled={this.state.optionButton} onClick={this.handleDisplay} style={{marginLeft:"45%"}}/>);
};
